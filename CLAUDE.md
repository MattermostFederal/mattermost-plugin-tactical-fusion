...
| The `cvedetail.tsv` and `cwedetail.tsv` row shapes, between `build/cyberdata` and `intel` | `intel/testdata/cvedetail.tsv` and `cwedetail.tsv`, which the generator must reproduce and the reader must parse |
