...
`plugin.json` declares fourteen switches: `EnableDTG` (the date-time group
...
`EnableLocationPlusCode` and `EnableLocationMoniker`. Mattermost's settings
schema has no nesting, so the grouping is by ordering and naming; the parent is
enforced in code, in `Plugin.dtgFormats` and `Plugin.locationFormats`.

**Thirteen default on. `EnableLocationUTM` defaults off, and is the only one.** The
...
- **Do not comment the code.** Match the style of surrounding code in
  ...
  removed. See [Where the rationale goes](#where-the-rationale-goes) below.
