...
| The `cvedetail.tsv`, `cwedetail.tsv` and `attackdetail.tsv` row shapes, between `build/cyberdata` and `intel` | The same-named files in `intel/testdata/`, which the generator must reproduce and the reader must parse |
