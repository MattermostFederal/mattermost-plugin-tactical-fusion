...
coastline that may be five kilometres from where it is gets drawn at street
scale with nothing saying so. A second archive, `public/map/detail.pmtiles`,
answers that with OpenStreetMap in the **OpenMapTiles schema**, generated by
`build/maposm/` and drawn from z10 up. The decisions inside the generator are in
...
band and in the other it is the same road drawn twice, kilometres apart, which
...
by name. OpenMapTiles has no land polygon: it draws water over a land-coloured
...
coastline and nothing else. The argument for that: five kilometres of source
...
It tests the **viewport against the package bounds, not the centre against
them**, and the asymmetry is the point. A centre test lifts the cap as soon as
...
accurate one still draws Oahu: the same road twice, kilometres apart, which is
...
**Attribution is now a licence condition, which reverses a documented decision.**
...
`public/map/LICENSE-OSM.txt` ships beside the archive and `make bundle` refuses a
bundle carrying one without the other, exactly as it does for the fonts. ODbL
...
**Regions are a build-time list, not a runtime package system**, which is what
keeps this feature down to two files in Go and none of them new. See
`build/maposm/README.md` for that and for the three things it leaves unresolved:
planetiler's ~1.2 GB of auxiliary datasets are pinned by nothing, a region cut
from a larger PBF needs a buffered `osmium extract` that is not built yet, and an
island pilot cannot test the buffer at all.
...
vertices hundreds of kilometres apart, `filter.json` renders `admin_level <= 4`,
