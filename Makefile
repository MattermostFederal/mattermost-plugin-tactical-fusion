...
## bundled KEV, CWE detail, ATT&CK detail and KEV-only CVE files into the tree, and the large ones into build/cyberdata/out for release
...
ifneq ($(HAS_SERVER),)
	...
	$(GO) vet ./server/... ./bridgeclient/...
	$(GOBIN)/golangci-lint run ./server/... ./bridgeclient/...
endif
